...
export class BookBrowserComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  protected sidebarFilterTogglePrefService = inject(
  ...
  protected bookCardOverlayPreferenceService = inject(
    BookCardOverlayPreferenceService,
  );
  protected bookSelectionService = inject(BookSelectionService);
  protected bookPatchService = inject(BookPatchService);
  ...
  mobileColumnCount = 3;
  selectedCount = 0;
  // true = 全部已打印, false = 全部未打印, null = 混合状态（点击→标记为已打印）
  selectedPrintedState: boolean | null = null;
  ...
  private sideBarFilter = new SideBarFilter(
    this.selectedFilter,
  ...
  get viewIcon(): string {
    return this.currentViewMode === VIEW_MODES.GRID
  ...
  get isFilterActive(): boolean {
    return (
      ...
      Object.keys(this.selectedFilter.value).length > 0
    );
  }
  ...
  get computedFilterLabel(): string {
    ...
    if (filterEntries.length === 1) {
      ...
      if (values.length === 1) {
        const displayValue = FilterLabelHelper.getFilterDisplayValue(
          filterType,
          values[0],
        );
    ...
    const filterSummary = filterEntries
      .map(
        ([type, values]) =>
          `${FilterLabelHelper.getFilterTypeName(type)} (${values.length})`,
      )
      .join(", ");

    return filterSummary.length > 50
      ? this.t.translate("book.browser.labels.activeFilters", {
  ...
  get isAudiobookOnlyLibrary(): boolean {
    ...
    return (
      ...
      library.allowedFormats.length === 1 &&
      library.allowedFormats[0] === "AUDIOBOOK"
    );
  }

  get seriesViewEnabled(): boolean {
    return Boolean(
      this.userService.getCurrentUser()?.userSettings?.enableSeriesView,
    );
  }
  ...
  ngAfterViewInit(): void {
    if (this.bookFilterComponent) {
      ...
      this.bookFilterComponent.selectedFilterMode =
  ...
  private getScrollPositionKey(): string {
    ...
    return this.scrollService.createKey(
      path,
  ...
  private saveScrollPosition(): void {
    if (this.virtualScroller?.viewPortInfo) {
      ...
      const position =
        this.virtualScroller.viewPortInfo.scrollStartPosition ?? 0;
  ...
  private initializeEntityRouting(): void {
    ...
    if (currentPath === "all-books" || currentPath === "unshelved-books") {
      ...
      const routeEntityInfo$ = this.entityService.getEntityInfoFromRoute(
        this.activatedRoute,
      );
      ...
      this.entity$ = routeEntityInfo$.pipe(
        switchMap(({ entityId, entityType }) =>
          this.entityService.fetchEntity(entityId, entityType),
        ),
      );
  ...
  private setupQueryParamSubscription(): void {
    combineLatest([
      ...
      this.currentFilterLabel = this.t.translate(
      ...
      this.entityViewPreferences =
        user.user?.userSettings?.entityViewPreferences;
      this.coverScalePreferenceService.initScaleValue(
        this.coverScalePreferenceService.scaleFactor,
      );
      this.columnPreferenceService.initPreferences(
        user.user?.userSettings?.tableColumnPreference,
      );
      this.visibleColumns = this.columnPreferenceService.visibleColumns;

      const visibleFields =
        user.user?.userSettings?.visibleSortFields ??
        DEFAULT_VISIBLE_SORT_FIELDS;
      const sortOptionsByField = new Map(
        this.bookSorter.sortOptions.map((o) => [o.field, o]),
      );
      this.visibleSortOptions = visibleFields
        .map((f) => sortOptionsByField.get(f))
      ...
      const sortNotYetEstablished =
        this.bookSorter.selectedSortCriteria.length === 0;
      if (
        (sortFromUrl || sortNotYetEstablished) &&
        !this.areSortCriteriaEqual(
          this.bookSorter.selectedSortCriteria,
          parseResult.sortCriteria,
        )
      ) {
    ...
    this.bookSelectionService.selectedBooks$
      ...
      .subscribe((selectedBooks) => {
        ...
        if (selectedBooks.size > 0) {
          const currentBooks =
            this.bookService.getCurrentBookState().books || [];
          const selected = currentBooks.filter((b) => selectedBooks.has(b.id));
          const printedCount = selected.filter(
            (b) => b.printed === true,
          ).length;
          if (printedCount === selected.length) {
            this.selectedPrintedState = true; // 全部已打印
          } else if (printedCount === 0) {
            this.selectedPrintedState = false; // 全部未打印
          } else {
            this.selectedPrintedState = null; // 混合
          }
        } else {
          this.selectedPrintedState = null;
        }
  ...
  onFilterSelected(filters: Record<string, any> | null): void {
    ...
    this.currentFilterLabel = hasSidebarFilters
  ...
  onFilterModeChanged(mode: BookFilterMode): void {
    if (
      this.settingFiltersFromUrl ||
      mode === this.selectedFilterMode.getValue()
    )
      return;
  ...
  updateScale(): void {
    this.coverScalePreferenceService.setScale(
      this.coverScalePreferenceService.scaleFactor,
    );
  }

  onVisibleColumnsChange(selected: { field: string; header: string }[]): void {
    const allFields = this.bookTableComponent.allColumns.map(
      (col) => col.field,
    );
  ...
  confirmDeleteBooks(): void {
    this.confirmationService.confirm({
      message: this.t.translate("book.browser.confirm.deleteMessage", {
      ...
      accept: () => {
        ...
        const loader = this.loadingService.show(
          this.t.translate("book.browser.loading.deleting", { count }),
        );

        this.bookService
          .deleteBooks(this.selectedBooks)
  ...
  applySortCriteria(sortCriteria: SortOption[]): void {
    ...
    if (this.entityType === EntityType.ALL_BOOKS) {
      this.bookState$ = this.entityService.fetchAllBooks(primarySort).pipe(
        map((bookState) =>
          this.applyClientSideMultiSort(bookState, sortCriteria),
        ),
        switchMap((bookState) => this.applyBookFilters(bookState)),
      );
    } else if (this.entityType === EntityType.UNSHELVED) {
      this.bookState$ = this.entityService
        .fetchUnshelvedBooks(primarySort)
        .pipe(
          map((bookState) =>
            this.applyClientSideMultiSort(bookState, sortCriteria),
          ),
      ...
      const routeParam$ = this.entityService.getEntityInfoFromRoute(
        this.activatedRoute,
      );
      this.bookState$ = routeParam$.pipe(
        switchMap(({ entityId, entityType }) =>
          this.entityService.fetchBooksByEntity(
            entityId,
            entityType,
            primarySort,
          ),
  ...
  private applyClientSideMultiSort(
    bookState: BookState,
  ...
  private areSortCriteriaEqual(a: SortOption[], b: SortOption[]): boolean {
    ...
    return a.every(
      (criterion, index) =>
        criterion.field === b[index].field &&
        criterion.direction === b[index].direction,
    );
  }
    ...
    if (
      ...
      switch (this.entityType) {
        case EntityType.LIBRARY:
          overrideEntityType = "LIBRARY";
          break;
        case EntityType.SHELF:
          overrideEntityType = "SHELF";
          break;
        case EntityType.MAGIC_SHELF:
          overrideEntityType = "MAGIC_SHELF";
          break;
        default:
          return;
      }
      ...
      const existingIndex = prefs.overrides.findIndex(
        (o) =>
          o.entityType === overrideEntityType && o.entityId === this.entity!.id,
      );
    ...
    this.messageService.add({
      ...
      detail:
        ...
        this.entityType === EntityType.UNSHELVED
          ...
          : this.t.translate("book.browser.toast.sortSavedEntityDetail", {
  ...
  toggleTableGrid(): void {
    this.currentViewMode =
      this.currentViewMode === VIEW_MODES.GRID
        ? VIEW_MODES.TABLE
        : VIEW_MODES.GRID;
    this.queryParamsService.updateViewMode(
      this.currentViewMode as "grid" | "table",
    );
  }
  ...
  unshelfBooks(): void {
    ...
    const count = this.selectedBooks.size;
    const loader = this.loadingService.show(
      this.t.translate("book.browser.loading.unshelving", { count }),
    );
      ...
      .subscribe({
        next: () => {
          this.messageService.add({
            ...
            detail: this.t.translate(
              "book.browser.toast.unshelveSuccessDetail",
            ),
          });
          this.bookSelectionService.deselectAll();
        },
        error: () => {
          this.messageService.add({
            ...
            detail: this.t.translate("book.browser.toast.unshelveFailedDetail"),
          });
        },
      });
  }
  ...
  openShelfAssigner(): void {
    this.dynamicDialogRef = this.dialogHelperService.openShelfAssignerDialog(
      null,
      this.selectedBooks,
    );
  ...
  lockUnlockMetadata(): void {
    this.dynamicDialogRef =
      this.dialogHelperService.openLockUnlockMetadataDialog(this.selectedBooks);
    if (this.dynamicDialogRef) {
  ...
  bulkEditMetadata(): void {
    this.dynamicDialogRef = this.dialogHelperService.openBulkMetadataEditDialog(
      this.selectedBooks,
    );
    if (this.dynamicDialogRef) {
      this.dynamicDialogRef.onClose.subscribe(() => {
  ...
  regenerateCoversForSelected(): void {
    ...
    const count = this.selectedBooks.size;
    this.confirmationService.confirm({
      message: this.t.translate("book.browser.confirm.regenCoverMessage", {
        count,
      }),
      ...
      accept: () => {
        this.bookMetadataManageService
          .regenerateCoversForBooks(Array.from(this.selectedBooks))
          .subscribe({
            next: () => {
              this.messageService.add({
                severity: "success",
                summary: this.t.translate(
                  "book.browser.toast.regenCoverStartedSummary",
                ),
                detail: this.t.translate(
                  "book.browser.toast.regenCoverStartedDetail",
                  { count },
                ),
                life: 3000,
              });
            ...
            error: () => {
              this.messageService.add({
                ...
                detail: this.t.translate(
                  "book.browser.toast.regenCoverFailedDetail",
                ),
                life: 3000,
              });
            },
  ...
  generateCustomCoversForSelected(): void {
    ...
    const count = this.selectedBooks.size;
    this.confirmationService.confirm({
      message: this.t.translate("book.browser.confirm.customCoverMessage", {
        count,
      }),
      ...
      accept: () => {
        this.bookMetadataManageService
          .generateCustomCoversForBooks(Array.from(this.selectedBooks))
          .subscribe({
            next: () => {
              this.messageService.add({
                severity: "success",
                summary: this.t.translate(
                  "book.browser.toast.customCoverStartedSummary",
                ),
                detail: this.t.translate(
                  "book.browser.toast.customCoverStartedDetail",
                  { count },
                ),
                life: 3000,
              });
            ...
            error: () => {
              this.messageService.add({
                ...
                summary: this.t.translate("book.browser.toast.failedSummary"),
                detail: this.t.translate(
                  "book.browser.toast.customCoverFailedDetail",
                ),
                life: 3000,
              });
            },
  ...
  attachFilesToBook(): void {
    ...
    if (libraryIds.size > 1) {
      this.messageService.add({
        severity: "warn",
        summary: this.t.translate(
          "book.browser.toast.multipleLibrariesSummary",
        ),
    ...
    this.dynamicDialogRef =
      this.dialogHelperService.openBulkBookFileAttacherDialog(sourceBooks);
    if (this.dynamicDialogRef) {
      this.dynamicDialogRef.onClose.subscribe((result) => {
        if (result?.success) {
          this.bookSelectionService.deselectAll();
  ...
  bulkTogglePrintedStatus(): void {
    if (!this.selectedBooks || this.selectedBooks.size === 0) return;
    // 全部已打印时→取消打印；其余情况（未打印/混合）→标记为已打印
    const newStatus = this.selectedPrintedState !== true;
    const ids = Array.from(this.selectedBooks);
    const requests = ids.map((id) =>
      this.bookPatchService.updatePrintedStatus(id, newStatus),
    );
    import("rxjs").then(({ forkJoin }) => {
      forkJoin(requests).subscribe({
        next: () => {
          this.selectedPrintedState = newStatus;
          this.cdr.detectChanges();
        },
        error: (err) => console.error("批量更新打印状态失败", err),
      });
    });
  }
  ...
  private applyBookFilters(bookState: BookState): Observable<BookState> {
    const forceExpandSeries =
      this.filterOrchestrationService.shouldForceExpandSeries(
  ...
  private loadMobileColumnsPreference(): void {
    const saved = this.localStorageService.get<number>(
      this.MOBILE_COLUMNS_STORAGE_KEY,
    );
